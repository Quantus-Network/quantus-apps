    ...
    switch (field) {
      ...
      case AmountField(:final label, :final planck, :final assetId, :final note):
        final value = assetId == null
            ? ref.watch(txAmountDisplayProvider)(planck, isSend: true).primaryAmount
            : '$planck (asset #$assetId, raw units)';
        return Padding(
          padding: const EdgeInsets.only(top: 6),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
...
class DecodedCallHeadline {
  ...
  /// [amountText] formats a native planck amount for the current locale and
  /// currency display; injected so this stays independent of Riverpod.
  static DecodedCallHeadline of(DecodedCall call, {required String Function(BigInt planck) amountText}) {
