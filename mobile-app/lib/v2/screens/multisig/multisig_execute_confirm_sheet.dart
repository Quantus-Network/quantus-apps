...
void showMultisigExecuteConfirmSheet(
  ...
  BottomSheetContainer.show(
    context,
    builder: (_) => _MultisigExecuteConfirmSheet(msig: msig, proposal: proposal),
  );
}

class _MultisigExecuteConfirmSheet extends ConsumerStatefulWidget {
  final MultisigAccount msig;
  final MultisigProposal proposal;

  const _MultisigExecuteConfirmSheet({required this.msig, required this.proposal});

  @override
  ConsumerState<_MultisigExecuteConfirmSheet> createState() => _MultisigExecuteConfirmSheetState();
}

class _MultisigExecuteConfirmSheetState extends ConsumerState<_MultisigExecuteConfirmSheet> {
  ...
  Future<void> _loadNetworkFee() async {
    ...
    try {
      ...
      final fee = await ref
          ...
          .estimateExecuteFee(msig: widget.msig, signer: signer, proposalId: widget.proposal.id);
      ...
      debugPrint('Execute fee estimate error: $e $st');
      if (!mounted) return;
  ...
  Future<void> _confirm() async {
    ...
    final authed = await LocalAuthService().authenticate(localizedReason: l10n.multisigExecuteAuthReason);
    if (!authed || !mounted) {
      setState(() {
        _submitting = false;
    ...
    try {
      ...
      debugPrint('Execute submit error: $e $st');
      if (!mounted) return;
      setState(() {
        _submitting = false;
        _errorMessage = l10n.multisigExecuteFailed;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BottomSheetContainer(
      title: l10n.multisigExecuteConfirmTitle,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        mainAxisSize: MainAxisSize.min,
        children: [
          const SizedBox(height: 16),
          Text(l10n.multisigExecuteConfirmBody, style: text.paragraph?.copyWith(color: colors.textPrimary)),
          ...
          QuantusButton.simple(
            label: l10n.multisigExecuteConfirmYes,
