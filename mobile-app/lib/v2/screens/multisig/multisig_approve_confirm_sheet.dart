...
void showMultisigApproveConfirmSheet(
  ...
  BottomSheetContainer.show(
    context,
    builder: (_) => _MultisigApproveConfirmSheet(msig: msig, proposal: proposal),
  );
}

class _MultisigApproveConfirmSheet extends ConsumerStatefulWidget {
  final MultisigAccount msig;
  final MultisigProposal proposal;

  const _MultisigApproveConfirmSheet({required this.msig, required this.proposal});

  @override
  ConsumerState<_MultisigApproveConfirmSheet> createState() => _MultisigApproveConfirmSheetState();
}
...
class _MultisigApproveConfirmSheetState extends ConsumerState<_MultisigApproveConfirmSheet> {
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      setState(() {
        _submitting = false;
        _errorMessage = l10n.multisigApproveFailed;
      });
    }
  }
