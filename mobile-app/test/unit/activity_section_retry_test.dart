...
void main() {
  ...
  Future<void> pumpErrored(WidgetTester tester, {required bool online, Future<void> Function()? onRetry}) =>
      tester.pumpApp(
        ActivitySection(
  ...
  testWidgets('offline, the error offers no Retry since a refresh would be skipped', (tester) async {
    await pumpErrored(tester, online: false);
  ...
  testWidgets('Retry shows the loading skeleton until the reload settles', (tester) async {
    ...
    await pumpErrored(tester, online: true, onRetry: () => reload.future);
