...
void main() {
  ...
  testWidgets('active step without progress has no substate', (tester) async {
    await pumpSteps(tester, currentStep: 1);

    final row = rows(tester).singleWhere((r) => r.label == 'Preparing');
    expect(row.state, StepRowState.active);
    expect(row.substate, isNull);
  });
  ...
  testWidgets('hasError on the current step maps to ended without a substate', (tester) async {
  ...
  testWidgets('hasError marks only the current step', (tester) async {
    await pumpSteps(
      tester,
      currentStep: 6,
      hasError: true,
      stepProgress: {
        4: item(4, completed: 2, total: 4),
        5: item(5, completed: 1, total: 2),
        6: item(6, completed: 0, total: 2),
      },
    );

    final states = {for (final r in rows(tester)) r.label: r.state};
    expect(states['Generate proofs'], StepRowState.pending);
    expect(states['Building privacy proof'], StepRowState.pending);
    expect(states['Submitting to chain'], StepRowState.ended);
  });

  testWidgets('cancelled in-flight current step maps to pending', (tester) async {
    await pumpSteps(tester, currentStep: 5, cancelled: true, stepProgress: {5: item(5, completed: 1, total: 3)});

    final row = rows(tester).singleWhere((r) => r.label == 'Building privacy proof');
    expect(row.state, StepRowState.pending);
    expect(row.substate, isNull);
  });
