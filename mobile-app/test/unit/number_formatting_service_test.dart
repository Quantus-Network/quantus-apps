  ...
  group('NumberFormattingService', () {
    ...
    group('formatBalance', () {
      ...
      test('formats balance less than 1 unit', () {
        final balance = BigInt.parse('500000000'); // 0.5
      ...
      test('formats balance with exactly max decimals', () {
        final balance = BigInt.parse('1234500000'); // 1.2345
      ...
      test('formats balance with more decimals than max (rounds display)', () {
        final balance = BigInt.parse('1234567800'); // 1.2345678
      ...
      test('formats balance with fewer decimals than max', () {
        final balance = BigInt.parse('1200000000'); // 1.2
      ...
      test('formats large balance', () {
        final balance = BigInt.parse('1234567890123000000'); // 1,234,567,890.123
      ...
      test('formats large balance without decimals', () {
        final balance = BigInt.parse('1234567890000000000'); // 1,234,567,890
      ...
      test('formats minimal decimal value', () {
        ...
        expect(service.formatBalance(balance, maxDecimals: 12), '0.000000001');
    ...
    group('parseAmount', () {
      ...
      test('parses decimal string', () {
        expect(service.parseAmount('1.2345'), BigInt.parse('1234500000'));
        expect(service.parseAmount('0.5'), BigInt.parse('500000000'));
      });

      test('parses decimal string with max decimals', () {
        expect(service.parseAmount('1.123456789'), BigInt.parse('1123456789'));
      });

      test('parses decimal string exceeding max decimals (truncates)', () {
        expect(service.parseAmount('1.123456789999'), BigInt.parse('1123456789'));
      });

      test('parses string starting with decimal point', () {
        expect(service.parseAmount('.5'), BigInt.parse('500000000'));
      });
