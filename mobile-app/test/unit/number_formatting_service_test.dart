  ...
  group('NumberFormattingService', () {
    ...
    group('formatBalance', () {
      ...
      test('formats balance with exactly max decimals', () {
        ...
        expect(
          service.formatBalance(balance, maxDecimals: 5),
          '1.2345',
        ); // No trailing zeros
      });

      test(
        'formats balance with more decimals than max (truncates display)',
        () {
      ...
      test('formats large balance', () {
        ...
        expect(
          service.formatBalance(balance, maxDecimals: 4),
          '1,234,567,890.123',
        );
      });
      ...
      test('formats minimal decimal value', () {
        ...
        expect(
          service.formatBalance(balance, maxDecimals: 12),
          '0.000000000001',
        );
        expect(
          service.formatBalance(balance, maxDecimals: 4),
    ...
    group('parseAmount', () {
      ...
      test('parses decimal string with max decimals', () {
        expect(
          service.parseAmount('1.123456789'),
          BigInt.parse('1123456789000'),
        );
      });

      test('parses decimal string exceeding max decimals (truncates)', () {
        expect(
          service.parseAmount('1.123456789999999'),
          BigInt.parse('1123456789999'),
        );
      });
      ...
      test('parses string starting with decimal point, roundtrip test', () {
        ...
        expect(
          service.parseAmount('.5'),
          BigInt.parse(zeroPointFiveString12Digits),
        );
        expect(
          service.formatBalance(BigInt.parse(zeroPointFiveString12Digits)),
          '0.5',
        );
      });
