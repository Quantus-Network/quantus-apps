...
ChainRewards _rewards(TestnetChain chain, List<AddressReward> rows) => ChainRewards(chain: chain, rows: rows);

AddressReward _row(String address, {required int blocks, required int rewardHundredths, bool claimable = true}) =>
    AddressReward(
      match: AirdropMatch(address: address, kind: 'dilithium', scheme: 's', claimable: claimable, source: 'hd'),
      reward: MinerReward(blocks: blocks, rewardHundredths: rewardHundredths),
    );
...
void main() {
  ...
  testWidgets('each chain spins until it answers; the claim and total appear once all four have', (tester) async {
    ...
    pending[TestnetChain.resonance]!.complete(
      _rewards(TestnetChain.resonance, [
        _row('qzresonanceEarly${'a' * 36}', blocks: 100, rewardHundredths: 6039),
        _row('qzresonanceLate${'b' * 37}', blocks: 20, rewardHundredths: 500),
      ]),
    );
    pending[TestnetChain.schrodinger]!.complete(
      _rewards(TestnetChain.schrodinger, [_row('qzschrodinger${'x' * 39}', blocks: 1, rewardHundredths: 10)]),
    );
    await tester.pump();
    ...
    expect(find.text('60.39 QTC'), findsOneWidget);
    expect(find.text('5 QTC'), findsOneWidget);
    expect(find.text('1 block mined'), findsOneWidget);
    expect(find.text('0.1 QTC'), findsNWidgets(2));
    for (final address in ['qzresonanceEarly${'a' * 36}', 'qzresonanceLate${'b' * 37}']) {
      expect(find.text(AddressFormattingService.formatAddress(address)), findsOneWidget);
    }
    expect(find.text(l10n.miningRewardsTotal), findsNothing);
    expect(claimEnabled(tester), isFalse);

    pending[TestnetChain.dirac]!.complete(_rewards(TestnetChain.dirac, []));
    pending[TestnetChain.planck]!.complete(
      _rewards(TestnetChain.planck, [
        _row('qzplanckRate4${'x' * 39}', blocks: 7, rewardHundredths: 300, claimable: false),
      ]),
    );
    await tester.pump();
    expect(find.byType(Loader), findsNothing);
    expect(find.text('0 blocks mined'), findsOneWidget);
    expect(find.text('7 blocks mined'), findsOneWidget);
    expect(find.text(l10n.miningRewardsNotClaimable), findsOneWidget);
    expect(find.text('3 QTC'), findsNothing);
  ...
  testWidgets('a failed chain offers a retry and keeps the claim closed', (tester) async {
    ...
    for (final chain in TestnetChain.values.skip(1)) {
      pending[chain]!.complete(_rewards(chain, [_row('qz${chain.name}', blocks: 500, rewardHundredths: 100)]));
    }
