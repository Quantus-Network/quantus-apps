...
void main() {
  ...
  testWidgets('shows QR, address, and checkphrase on one screen without tabs', (tester) async {
    ...
    expect(find.text(l10n.receiveTabQrCode), findsNothing);
    expect(find.text(l10n.receiveTabAddress), findsNothing);
  ...
  testWidgets('uses v3 tokens and stacked primary copy / secondary share', (tester) async {
    ...
    expect(address.style?.color, colors.textContent);
    ...
    expect(buttonWithLabel(tester, l10n.componentShare).variant, ButtonVariant.secondary);
  });
}
