...
class WormholeSendService {
      ...
      final spends = [
        for (final transfer in unspent)
          WormholeLeafSpend(
            ...
            outputAmount1: wormholeNetScaled(wormholeScaledFromRaw(transfer.amount)),
          ),
      ];
      final batches = [
    ...
    try {
      ...
      return ClaimResult(
        totalWithdrawn: submitted.fold(
          BigInt.zero,
          (sum, b) => sum + b.fold(BigInt.zero, (s, spend) => s + wormholeRawFromScaled(spend.outputAmount1)),
        ),
  ...
  /// the output buffers. Returns the raw amount paid to exit slot 1.
  ...
  Future<BigInt> _generateLeafProof({
    ...
    onComplete?.call();
    // On-chain dispatch transfers `outputAmount * scaleFactor` raw units to
    // each exit account; slot 1 is the recipient's exact contribution.
    return wormholeRawFromScaled(spend.outputAmount1);
  }
