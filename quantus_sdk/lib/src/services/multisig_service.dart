...
import 'package:quantus_sdk/generated/bell/pallets/multisig.dart' show Constants, Txs;
import 'package:quantus_sdk/generated/bell/bell.dart' show Bell;
...
class MultisigService {
  ...
  Future<List<MultisigProposal>> _withOnChainCallData(MultisigAccount msig, List<MultisigProposal> proposals) async {
    ...
    try {
      final queries = Bell(provider).query.multisig;
  ...
  Future<List<int>> fetchProposalCallBytes({required MultisigAccount msig, required int proposalId}) async {
    ...
    final stored = await Bell(provider).query.multisig.proposals(getAccountId32(msig.accountId), proposalId);
    if (stored == null) {
