...
/// Walks a [DecodedCall] tree: title, then every [CallField], nested calls in a card.
///
/// Presentational only. Callers supply [titleOf] and [formatAmount]. [addressBuilder]
/// replaces address [ValueField]s (e.g. a checkphrase row); omit it to render the
/// ss58 string. [nestedCallBuilder] replaces the default recursive [DecodedCallView]
/// so a host can keep a hero summary inside nested cards.
...
class CallFieldView extends StatelessWidget {
  ...
  Widget _amount(BuildContext context, AmountField field) {
    return _row(context, label: field.label, value: formatAmount(field), note: amountNoteOf?.call(field) ?? field.note);
  }
        ...
        children: [
          ...
          if (field.note != null)
            Padding(
              ...
              child: Text(
                field.note!,
                style: text.caption.copyWith(color: stacked ? colors.textMuted : colors.textMuted2),
              ),
            ),
        ],
      ),
