...
class Queries {
  ...
  final _i1.StorageValue<BigInt> _nextTransactionId =
  ...
  /// guardian.
  _i7.Future<List<_i2.AccountId32>> guardianIndex(
    _i2.AccountId32 key1, {
    _i1.BlockHash? at,
  }) async {
  ...
  /// This allows the UI to efficiently query all accounts for which a given account is a
  /// guardian.
  _i7.Future<List<List<_i2.AccountId32>>> multiGuardianIndex(
    List<_i2.AccountId32> keys, {
    _i1.BlockHash? at,
  }) async {
    final hashedKeys =
        keys.map((key) => _guardianIndex.hashedKeyFor(key)).toList();
    final bytes = await __api.queryStorageAt(
      hashedKeys,
      at: at,
    );
    if (bytes.isNotEmpty) {
      return bytes.first.changes
          .map((v) => _guardianIndex.decodeValue(v.key))
          .toList();
    }
    return (keys.map((key) => []).toList()
